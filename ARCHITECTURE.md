...
> 五测（2026-07）后更新：三实验均开 HM、各 1200 轮；实验1·HIO 重复 3 次（收敛概率性）；relaxed HIO 修复尺度爆炸。项目背景见 [README.md](README.md)，理论见 [调研报告.md](调研报告.md)，五测发现见 [五测汇报.md](五测汇报.md)。
...
## 8. 已知问题（五测状态，详见 [实验猜想与结论.md](实验猜想与结论.md)）

1. **HIO 收敛概率性**（C2/C10）：收敛→轮廓、不收敛→周期斜线，与迭代长度无单调关系。对策：实验1 重复 3 次。
2. **~~support 自锁~~（C7 已推翻）**：sigmoid 长跑慢相变收敛（ssim 0.95），非死锁。
3. **尺度发散已修复**（C8）：relaxed HIO（γ=0.9）使 tanh+HIO 末轮 psnr 不再 −38。
4. **twin image**（C11，待查）：tanh+HIO 结果上下半轮廓偏移。
5. **CUDA 非确定性**（C6）：UNet 不可复现，需多次跑取统计。
6. **amp_cc/ssim 局限**（C9/R5）：amp_cc 与形态脱钩；ssim 在 HIO"形态+斜线"场景失真。以看图为准。
